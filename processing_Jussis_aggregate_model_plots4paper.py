...
'''
tumbling=True #so far only the use of non-tumbling projected area is implemented here
...
for particle_type in particle_types:
    ...
    particle_dic,N_mono_list = __tools_for_processing_Jagg.read_particle_prop_files(    prop_file_folder = "/data/optimice/Jussis_aggregates_bugfixedrotation/",
    ...
    for i_prop,(prop,prop_unit,prop_short) in enumerate(zip(["mass","area"],["kg","m2"],["m","A"])):
        ...
        fitlines = axes[i_ax].plot(fit_dic["diam"],fit_dic[prop + "_Nmono_allagg"],c="g",linewidth=2.*linewidth,label="all aggregates")
        ...
        for i_Nmono,N_mono_now in enumerate(Nmono_fit_eval):
            ...
            fitlines = axes[i_ax].plot(np.log10(diam_center),(prop_fitted[i_Nmono,:]),c=usecolorsN[N_mono_now==N_mono_list][0],linewidth=linewidth,linestyle="-")
            
        #also plot all aggregates in this plot
        if prop=="mass":
            fitlines = axes[i_ax].plot(np.log10(fit_dic["diam"]),np.log10(fit_dic[prop + "_Nmono_allagg"]/(a*fit_dic["diam"]**b)),c="g",linewidth=2.*linewidth,label="all aggregates")
        elif prop=="area":
            fitlines = axes[i_ax].plot(np.log10(fit_dic["diam"]),np.log10(fit_dic[prop + "_Nmono_allagg"]/(c*fit_dic["diam"]**d)),c="g",linewidth=2.*linewidth,label="all aggregates")

        #make labels
        axes[i_ax].set_xlabel("log10(diameter D / m)")
    ...
    for i_prop,prop in enumerate(["vterm_bohm","vterm_HW10","vterm_KC05"]): #,"vterm_mitch_heym"]): #,"HWJussi","KCJussi"]):
        ...
        fitlines_allagg = axes[i_ax].semilogx(fit_dic["diam"],vterm_fit_allagg,c="g",linestyle="-",label="all aggregates",linewidth=2.*linewidth)
