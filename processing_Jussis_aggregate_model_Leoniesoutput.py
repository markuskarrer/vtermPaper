...
for particle_type in particle_types: 
    ...
    for mean_size in ["50","100","650","1000"]: #loop over the mean size of the distribution of the pristine crystals
        with open(prop_file_folder + particle_type +"_"+ mean_size +"_properties.txt","rb") as txtfile: #TODO: this is just one file! #http://effbot.org/zone/python-with-statement.htm explains what if is doing; open is a python build in
            ...
            for i_row,row_content in enumerate(prop_reader):
                ...
                particle_dic["HWJussi"] = np.append(particle_dic["HWJussi"],row_content[3])
                particle_dic["KCJussi"] = np.append(particle_dic["KCJussi"],row_content[4])
    ...
    for i_prop,prop in enumerate(["mass","area","vterm_HW10","vterm_KC05","vterm_bohm","HWJussi","KCJussi"]):
        ...
        for i_scalex,scalex in enumerate(["linear","log"]):
            for i_scaley,scaley in enumerate(["linear","log"]):
                ...
                im = axes[i_ax].scatter(diam_for_this_particle_type,prop_for_this_particle_type,s=1,c=Nmono_for_this_particle_type,rasterized=True,norm=colors.LogNorm(),cmap="gist_ncar")
                ...
                if scaley=="linear":
                    axes[i_ax].set_ylim([0,np.array([1e-5,1e-4,1.5,1.5,1.5,1.5,1.5])[i_prop]])
                #add colorbar
