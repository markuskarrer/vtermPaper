...
show_lines=dict()
powerlawAtlas="Atlas" #"Atlas","powerlaw" 
...
with open("/home/mkarrer/Dokumente/plots/fit_results_vterm.txt","w") as txtfile: #http://effbot.org/zone/python-with-statement.htm explains what if is doing; open is a python build in
    ...
    for particle_type in particle_types:
        ...
        for i_prop,prop in enumerate(["vterm_bohm","vterm_HW10","vterm_KC05"]): #,"vterm_mitch_heym"]): #,"HWJussi","KCJussi"]):
            ...
            fitlines_allagg = axes[i_ax].semilogx(fit_dic["diam"],fit_dic[prop +  "_fitted_via_mD_AD_Nmono1"],c="b",linestyle="-",label="Nmono=1",linewidth=linewidth)

            #add a line for the aggregates
            fitlines_allagg = axes[i_ax].semilogx(fit_dic["diam"],fit_dic[prop +  "_fitted_via_mD_AD_Nmonoallagg"],c="g",linestyle="-",label="Nmono>1",linewidth=linewidth)
            ...
            if show_lines["Atlas_fit"]:
                ...
                Atlasfit_monomersDeq = axes[i_ax].semilogx((np.pi*rhow*fit_dic["diam_wide"]**3/(6*fit_dic["mass_coeff_Nmono_1"][0]))**(1./fit_dic["mass_coeff_Nmono_1"][1]),fit_dic[prop +  "_Nmono_1_Deq"],c="b",linestyle="--",label="Atlas-type cl. ice (Deq)",linewidth=linewidth)

            if show_lines["powerlaw_fits"]:
                #power-law
                powerlaw_monomers, = axes[i_ax].semilogx(fit_dic["diam_full"], fit_dic[prop + "_Nmono_1_powerlaw"],marker='None',c="b",linestyle="-.",label="powerlaw Nmono=1",linewidth=linewidth)
            ...
            if show_lines["Atlas_fit"]:
                ...
                Atlasfit_NmonoallaggDeq = axes[i_ax].semilogx((np.pi*rhow*fit_dic["diam_wide"]**3/(6*fit_dic["mass_coeff_Nmono_allagg"][0]))**(1./fit_dic["mass_coeff_Nmono_allagg"][1]),fit_dic[prop +  "_Nmono_allagg" + "_Deq"],c="g",linestyle="--",label="Atlas-type " + corr_cat + " (Deq)",linewidth=linewidth)

            if show_lines["powerlaw_fits"]:
                #power-law
                powerlaw_handle, = axes[i_ax].semilogx(fit_dic["diam_full"], fit_dic[prop + "_Nmono_allagg" + "_powerlaw"],marker='None',c="g",linestyle="-.",label="powerlaw " + corr_cat,linewidth=linewidth)
