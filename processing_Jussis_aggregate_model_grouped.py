...
particle_types = ["needle"] #,"dendrite","plate","column","rosette"] # ,"bullet"rosette
for particle_type in particle_types:
    ...
    for filename in glob.glob(prop_file_folder + sensrun_folder +  particle_type + '*properties.txt'): #loop over all property files (with different mean sizes)
        ...
        with open(filename,"rb") as txtfile: #TODO: this is just one file! #http://effbot.org/zone/python-with-statement.htm explains what if is doing; open is a python build in
            ...
            if take_all_Nmono_bool:
                N_mono_groups = np.append(np.array(range(1,100)),np.array(range(125,1000,25)))
            else: #select certain monomer numbers
                N_mono_groups = np.array([[1,1],[2,10],[11,20],[21,50],[51,100]]) #define a matrix which contains intervals [] of selected monomer numbers
            #for N_mono_now in N_mono_groups: # magnitude in [1,10,100]: #itertools.islice (which is used for reading only certain rows in the txt files) just works with range() like input, so this is the workaround to be able to select all kind of monomer numbers
            ...
            for i_row,row_content in enumerate(prop_reader): #TODO: the row is not any more equal to the monomer number #read the row with N_mono_now (the currently considered monomer number)
                #from IPython.core.debugger import Tracer ; Tracer()()
                for i_N_mono_groups in range(0,N_mono_groups.shape[0]):
                    if row_content[0]>=N_mono_groups[i_N_mono_groups,0] and row_content[0]<=N_mono_groups[i_N_mono_groups,1]: #i_row==0 or i_row==4 or i_row==9:
                        particle_dic["particle_type"] = np.append(particle_dic["particle_type"],particle_type)
                        particle_dic["N_monomer"] = np.append(particle_dic["N_monomer"],N_mono_groups[i_N_mono_groups,1]) #python indices start with 0
    ...
    cmap = plt.cm.brg
    bounds = np.append(N_mono_groups[:,0],[9999]) #set the boundaries to the beginning of each interval
    norm = colors.BoundaryNorm(bounds, cmap.N)
    usecolors = pylab.cm.brg(np.linspace(0,1,N_mono_groups.shape[0]))
    ...
    #calculate the histogram and KDE for the monomer numbers in N_mono_groups
    for i,N_mono_now in enumerate(N_mono_groups[:,1]): #N_mono_now is the upper end of the grouped intervals 
    ...
    fitresult_arr_dir_powerlaw_string = np.zeros((5,N_mono_groups.shape[0],2)) #fit results (saved as floats) #dimensions: 0-> [mass,array]; 1-> monomer number; 2-> parameters (a,b) in fit
    ...
    for i_prop,(prop,prop_unit) in enumerate(zip(["mass","area"],["kg","m2"])): #,"HWJussi","KCJussi"]):
        ...
        axes[i_ax],particle_dic,fitline_allagg = __tools_for_processing_Jagg.fitting_wrapper(axes[i_ax],particle_dic,prop,N_mono_groups[:,1],usecolors,fit_dic,diam,function="powerlaw",linewidthinvalid=0.05,linewidthvalid=0.7,allagg=False) #N_mono_now is the upper end of the grouped intervals 
        ...
        cbar.set_ticklabels(N_mono_groups)
        #cbar.set_ticklabels
        
        #add labels for the fit result to the plot
        axes[i_ax] = __tools_for_processing_Jagg.add_fitresult_text(axes[i_ax],fit_dic,prop,N_mono_groups[:,1],function="powerlaw",hide_Nmono_fits=take_all_Nmono_bool,allagg=False,N_mono_listforlabel=N_mono_groups)
    ...
    for i_prop,prop in enumerate(["vterm_bohm","vterm_HW10","vterm_KC05","vterm_mitch_heym"]): #,"HWJussi","KCJussi"]):
        ...
        if Atlasfit_bool:
            ...
            for i,str_monomer_agg in enumerate(["1"]): # make a power-law fit for the pristine crystals and the aggregate
