...
for particle_type in particle_types:
    ...
    cmap = plt.cm.brg
    bounds = range(0,i_file)
    #from IPython.core.debugger import Tracer ; Tracer()()
    norm = colors.BoundaryNorm(bounds, cmap.N)
    usecolors = pylab.cm.brg(np.linspace(0,1,len(bounds)))
    ...
    for i,N_mono_now in enumerate(bounds): 
    ...
    for i_prop,(prop,prop_unit) in enumerate(zip(["mass","area"],["kg","m2"])): #,"HWJussi","KCJussi"]):
        ...
        im = axes[i_ax].scatter(particle_dic["diam"],particle_dic[prop],s=1,c=particle_dic["sizeparam_index"],rasterized=True,norm=norm,cmap=cmap)
        '''
        ...
        cbar.set_label("sizeparam_index")
