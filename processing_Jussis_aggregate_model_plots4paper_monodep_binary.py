...
with open("/home/mkarrer/Dokumente/plots/fit_results" + Dmax_side_add + ".txt","w") as txtfile: #http://effbot.org/zone/python-with-statement.htm explains what if is doing; open is a python build in
    ...
    for particle_type in particle_types:
        ...
        for i_prop,(prop,prop_unit,prop_short) in enumerate(zip(["mass","area","Aratio"],["kg","m2","1"],["m","A","Ar"])):
            ...
            fitlines = axes[i_ax].semilogx(np.nan,np.nan,linestyle="",label=prop_short + "-D power \nlaw fits",linewidth=linewidth)
            ...
            if not prop=="Aratio":
                ...
                axes[i_ax].set_ylabel((" / ").join(("area ratio " + prop_short,prop_unit))) 
            #change the axis
        ...
        #individual particle
        particle_dic["mABohm"] = particle_dic["mass"]/(a*particle_dic["diam"]**b)/(particle_dic["area"]/(particle_dic["diam"]**2*c*particle_dic["diam"]**d))**0.25
        particle_dic["mAHW10"] = particle_dic["mass"]/(a*particle_dic["diam"]**b)/(particle_dic["area"]/(particle_dic["diam"]**2*c*particle_dic["diam"]**d))**0.5
        particle_dic["mAKC"] = particle_dic["mass"]/(a*particle_dic["diam"]**b)/(particle_dic["area"]/(particle_dic["diam"]**2*c*particle_dic["diam"]**d))
        #fits
        ...
        for i_prop,(prop,prop_unit,prop_short) in enumerate(zip(["mABohm","mAHW10","mAKC"],[r"$ 1$",r"$1$",r"$1$"],[r"$\frac{(m/m_{mono})}{(A_r/A_{mono})^{0.25}}$",r"$\frac{(m/m_{mono})}{(A_r/A_{mono})^{0.5}}$",r"$\frac{(m/m_{mono})}{(A_r/A_{mono})}$"])):#,r"$(m/m_{mono})/(A/A_{mono})^{0.5}$","$(m/m_{mono})/(A/A_{mono})"])):
            ...
            im = axes2[i_ax].scatter(particle_dic["diam"],particle_dic[prop],s=1,c=particle_dic["N_monomer"],rasterized=True,norm=normN,cmap=cmapN) #all used particles        
        ...
        out_filestring2 = "mA_ratios_vsDfits_" + particle_type + '_' + tumbling_add
        
        plt.savefig(dir_save + out_filestring2 + '.pdf', dpi=400)
        plt.savefig(dir_save + out_filestring2 + '.png', dpi=100)
        print 'The pdf is at: ' + dir_save + out_filestring2 + '.pdf'
        ...
        for save_axis,ax_description in enumerate(ax_description_array):
            ...
            if "vterm" in ax_description:
                axes[save_axis].set_ylabel("terminal velocity $v_{term}$ / $ms^{-1}$")
