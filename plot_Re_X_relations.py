...
def Re_HW10(X):
    ...
    #print "c1",c1
    c2 = 0.25 * do_i**2
    bracket = np.sqrt(1.0 + c1*np.sqrt(X)) - 1.0
...
def Re_bohm(X):
    ...
    k = 1 #np.minimum(np.maximum(0.82+0.18*alpha,np.ones_like(alpha)*0.85),0.37+0.63/alpha,1.33/(np.maximum(np.log(alpha),np.ones_like(alpha)*0.0)+1.19)) #k is 1 for alpha=1
    #print "k",k
    #! 99 (11) or I (11)
    gama_big = 1 #np.maximum(np.ones_like(alpha)*1.0, np.minimum(np.ones_like(alpha)*1.98,3.76-8.41*alpha+9.18*alpha**2-3.53*alpha**3)) #1 for alpha=1
    #print "gama_big",gama_big
    #! 99 (10) or I (7)/(8)
    #C_DP = np.maximum(0.292*k*gama_big,0.492-0.2/np.sqrt(alpha)) #0.292 for alpha=1
    C_DP = 0.292 #np.maximum(1.0,q*(1.46*q-0.46))*C_DP #0.292 for alpha=1
    #print "C_DP",C_DP
    #! I (23) turbulence correction
...
def Re_bohm89(X):
    ...
    return 8.5*((1.0+0.1519*X**0.5)**0.5-1.0)**2
...
if __name__=="__main__": 
    ...
    Re_bohm89 = Re_bohm89(X)
    Re_bohm92 = Re_bohm(X)
    Re_HW10 = Re_HW10(X)
    Re_kc05,CD_kc05 = Re_kc05(X) #here C_D inludes the turbulence correction
    #turb_corr is only for mitch_heym
    #if velocity_model=='HW10':
    #    pass#vterm = vterm_hw10(mass,diam,area,rho=1.287,eta=1.717696e-5)
    #elif velocity_model=='KC05':
    #    pass#vterm = vterm_kc05(mass,diam,area,rho=1.287,eta=1.717696e-5) 
    #elif velocity_model=='bohm89':
    #elif velocity_model=='bohm':
    #    pass#vterm = vterm_bohm(mass,diam,area,np.ones_like(mass),rho=1.287,eta=1.717696e-5)
    #elif velocity_model=='bohmar':
    #    pass#vterm = vterm_bohm(mass,diam,area,as_ratio,rho=1.287,eta=1.717696e-5)
    #elif velocity_model=='mitch_heym': #the turbulence correction produces a jump at 0.5mm
    #    pass#vterm = vterm_mitch_heym(mass,diam,area,rho=1.287,eta=1.717696e-5,turb_corr=turb_corr)
    #else:
    ...
    if True:
    #if False:
        plt.loglog(X,Re_bohm89,label="B89")
        plt.loglog(X,Re_bohm92,label="B92")
        plt.loglog(X,Re_HW10,label="HW10")
        plt.loglog(X,Re_kc05,label="KC05",linestyle='--')
        plt.xlabel("X")
        plt.ylabel("Re")
        ...
        plt.savefig("/home/mkarrer/Dokumente/plots/hydromodels/ReX_bohm")
    #if True:
    #if False:
        plt.clf()
        CD_bohm89=X/Re_bohm89**2 
        CD_bohm92=X/Re_bohm92**2 
        CD_HW10= X/Re_HW10**2
        #CD_kc05= X/Re_kc05**2

        plt.loglog(X,CD_bohm89,label="B89")
        plt.loglog(X,CD_bohm92,label="B92")
        plt.loglog(X,CD_HW10,label="HW10")
        plt.loglog(X,CD_kc05,label="KC05",linestyle='--')
        plt.xlabel("X")
        plt.ylabel("C_d")
        ...
        ax2 = plt.twinx()  
        ax2.plot(X,CD_bohm92/CD_bohm89,color="r",label="B92/B89") 
         
        ax2.legend(loc="center right")

        plt.savefig("/home/mkarrer/Dokumente/plots/hydromodels/CdX_bohm")
    ...
    '''
    what Re range do we have?
    '''
    v_vec_B92=[0.1,1.0,1.25,1.25]    
    D_vec = [1e-4,1e-3,1e-2,4e-2] 
    Re_vec= [0]*4

    Re_vec[0]=C_atm*v_vec_B92[0]*D_vec[0]
    Re_vec[1]=C_atm*v_vec_B92[1]*D_vec[1]
    Re_vec[2]=C_atm*v_vec_B92[2]*D_vec[2]
    Re_vec[3]=C_atm*v_vec_B92[3]*D_vec[3]

    
    print "D_vec,Re_vec",D_vec,Re_vec
