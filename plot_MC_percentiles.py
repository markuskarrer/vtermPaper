    ...
    for i_sensMCfallspeed, sensrun_now_MC_fallspeed in enumerate(MCtermvel_list): #loop over different fall speed models (boehm, KC05,HW10,powerlaw, powerlawSB)
        ...
        for key in ["diam","vt"]:
            ...
            for i_quantile,quantile_now in enumerate(quantiles):
                axes[i_ax].plot(SP_percentiles[key][i_quantile,:],height_cent_vec,color=np.array(['r','b','g','y'])[i_sensMC+i_sensMCfallspeed],linestyle=np.array(["--","-","--"])[i_quantile],linewidth=np.array([0.5,1.5,0.5])[i_quantile])
                if key=="diam":
        ...
        for key in ["diam","vt"]:
            ...
            #axes[i_ax].semilogx(SP_percentiles[key][2,:],height_cent_vec,color=np.array(['b','r','g'])[i_sensMC+i_sensMCfallspeed])
            axes[i_ax].plot(SP_percentiles[key][2,:]-SP_percentiles[key][0,:],height_cent_vec,color=np.array(['r','b','g','y'])[i_sensMC+i_sensMCfallspeed],linestyle="-",linewidth=1.5)
            if key=="diam":
                axes[i_ax].set_xscale("linear")
            elif key=="vt":
                axes[i_ax].set_xscale("linear")
