...
for monotype in ["dendrite"]:
    ...
    fig,axes,im,tick_locs,N_mono_list = processing_Jussis_aggregate_model_plots4paper_monodep_binary.read_and_plot(fig,ax,[monotype],plot_vars=["vterm_KC05","vterm_HW10"])

    axes[0].set_ylabel("$v_{term}$ [m/s]")
    axes[1].set_ylabel("$v_{term}$ [m/s]")
    titles = [' ', ' ', ' ', ' ']
    ...
    out_filestring = "vD" + monotype +'_KC05HW10'
